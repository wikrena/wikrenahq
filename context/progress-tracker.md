...
- Should marketing pages here link out to Wikrena OS (`www.wikrenaos.com`) and Wikrena Consulting/AnalyticsHQ, given the three-pillar company structure described in `wikrena-pages/home.md` and `about-us.md`? Resolved for the homepage — yes, it already does this (Ecosystem section links to `www.wikrenaos.com` externally, `/academy` and `/services` locally). Apply the same pattern on future pages.
