...
import { CareerTracksPage } from "@/components/academy/tracks"
...
export default async function CareerTracks() {
  ...
  return (
    <AppShell userEmail={user.email ?? ""} userName={profile?.name} totalXp={totalXp} streak={profile?.current_streak ?? 0}>
      <CareerTracksPage enrollments={enrollments ?? []} userId={user.id} />
    </AppShell>
  )
}
