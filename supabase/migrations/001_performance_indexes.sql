...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lesson_completions_user_id
ON lesson_completions(user_id);

-- Find completions for a specific course (lesson player, course progress)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lesson_completions_course_id
ON lesson_completions(course_id);

-- Composite: student + course (most common join — lesson player)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lesson_completions_user_course
ON lesson_completions(user_id, course_id);

-- Recent completions (activity feed, parent reports)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lesson_completions_completed_at
ON lesson_completions(user_id, completed_at DESC);

-- ── COURSE ENROLLMENTS ────────────────────────────────────────────────────────

-- Find all enrollments for a student
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_course_enrollments_user_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_courses_junior_track
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lessons_chapter_id
ON lessons(chapter_id, "order");

-- Published lessons only
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lessons_chapter_published
ON lessons(chapter_id, is_published, "order")
WHERE is_published = true;

-- Lesson lookup by slug (lesson player URL routing)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lessons_slug
ON lessons(slug);

-- Chapters in a course (course page)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_chapters_course_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_parent_id
ON profiles(parent_id)
WHERE parent_id IS NOT NULL;

-- Active kids (Junior platform queries)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_kids
ON profiles(parent_id, is_kid)
WHERE is_kid = true;

-- Leaderboard sort (top students by XP)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_profiles_xp_leaderboard
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xp_transactions_user_id
ON xp_transactions(user_id, created_at DESC);

-- ── FORUM ────────────────────────────────────────────────────────────────────

-- Posts by category (forum category page)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_forum_posts_category_id
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_forum_replies_post_id
